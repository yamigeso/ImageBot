...
def detect_image_extension(data):
    """ファイルのバイトデータから画像の拡張子を判定する。画像でない場合は None。"""
...
async def on_message(message):
    ...
    for attachment in message.attachments:
        ...
        fixed_files.append(discord.File(io.BytesIO(data), filename=new_filename))
        print(f"画像を検出: {filename} -> {new_filename}")
...
if __name__ == "__main__":
    ...
    if os.environ.get("ENABLE_KEEP_ALIVE") == "1":
